...
func TestLeaderStartReplication2AB(t *testing.T) {
	...
	//fmt.Printf("node[%v],li = [%v], term = [%v]\n", r.id, li, r.RaftLog.LastLogTerm())
...
func TestLeaderAcknowledgeCommit2AB(t *testing.T) {
	tests := []struct {
		...
		{5, nil, false},						// error happened here!!!
...
func TestFollowerAppendEntries2AB(t *testing.T) {
	...
	for i, tt := range tests {
		...
		r := newTestRaft(1, []uint64{1, 2, 3}, 10, 1, storage)
		//fmt.Printf("raftLog=[%v], lastIndex=[%v], lastLogTerm=[%v], stable=[%v]\n", r.RaftLog.entries, r.RaftLog.LastIndex(), r.RaftLog.LastLogTerm(), r.RaftLog.stabled)
		r.becomeFollower(2, 2)
		r.Step(pb.Message{From: 2, To: 1, MsgType: pb.MessageType_MsgAppend, Term: 2, LogTerm: tt.term, Index: tt.index, Entries: tt.ents})
		//fmt.Printf("stable = [%v], lastIndex= [%v]\n", r.RaftLog.stabled, r.RaftLog.LastIndex())
...
func TestLeaderSyncFollowerLog2AB(t *testing.T) {
	...
	for i, tt := range tests {
		...
		//fmt.Printf("lead raftLog=[%v]\n", lead.RaftLog.entries)
		//fmt.Printf("foll raftLog=[%v]\n", follower.RaftLog.entries)
		//fmt.Println("----------------------------------------------")
...
func acceptAndReply(m pb.Message) pb.Message {
	...
	// debug : fmt.Printf("node reply, index=[%v]\n", m.Index + uint64(len(m.Entries)))
