...
func (d *storeWorker) handleMsg(msg message.Msg) {
	DPrintf3B("storeWorker is handling msg\n")
	switch msg.Type {
...
func (d *storeWorker) onRaftMessage(msg *rspb.RaftMessage) error {
	DPrintf3B("onRaftMessage is called!\n")
...
func (d *storeWorker) maybeCreatePeer(regionID uint64, msg *rspb.RaftMessage) (bool, error) {
	...
	DPrintf3B("maybeCreatePeer is called!\n")
	meta := d.ctx.storeMeta
	meta.Lock()
	defer meta.Unlock()
	if _, ok := meta.regions[regionID]; ok {
	...
	if !util.IsInitialMsg(msg.Message) {
		DPrintf3B("return2\n")
		DPrintf3B("message's type is [%v], commit=[%v]\n", msg.Message.MsgType, msg.Message.Commit)
		DPrintf3B("target peer %s doesn't exist\n", msg.ToPeer)
