...
func TestAllServerStepdown2AB(t *testing.T) {
	...
	for i, tt := range tests {
		...
		for j, msgType := range tmsgTypes {
			//fmt.Printf("node[%v] receive a [%v] message from node 2\n", sm.id, msgType)
...
func TestLeaderTransferToUpToDateNodeFromFollower3A(t *testing.T) {
	...
	lead := nt.peers[1].(*Raft)
	//fmt.Printf("lead's id : [%v]\n", lead.id)
	if lead.Lead != 1 {
		t.Fatalf("after election leader is %d, want 1", lead.Lead)
	}
	//fmt.Printf("node 2's role is [%v], leader is [%v]\n", nt.peers[2].(*Raft).State, nt.peers[2].(*Raft).Lead)
	// Transfer leadership to 2.
