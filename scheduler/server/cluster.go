...
func (c *RaftCluster) processRegionHeartbeat(region *core.RegionInfo) error {
	...
	//var versionGreat = false
	localRegion,_ := c.GetRegionByID(region.GetID())
	if localRegion != nil {
		 if !util.IsEpochStale(localRegion.RegionEpoch, region.GetRegionEpoch()) {
		 	return ErrRegionIsStale(region.GetMeta(), localRegion)
		 }
		 //versionGreat = region.GetRegionEpoch().Version > localRegion.RegionEpoch.Version || region.GetRegionEpoch().ConfVer > localRegion.RegionEpoch.ConfVer
	}else {
		localRegions := c.ScanRegions(region.GetStartKey(), region.GetEndKey(), -1)
		for _, curRegion := range localRegions {
			if !util.IsEpochStale(curRegion.GetRegionEpoch(), region.GetRegionEpoch()) {
				return ErrRegionIsStale(region.GetMeta(), curRegion.GetMeta())
			}
			/*versionCompare := region.GetRegionEpoch().Version > localRegion.RegionEpoch.Version || region.GetRegionEpoch().ConfVer > localRegion.RegionEpoch.ConfVer
			versionGreat = versionCompare || versionCompare*/
		}
	}
