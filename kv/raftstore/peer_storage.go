...
func NewPeerStorage(engines *engine_util.Engines, region *metapb.Region, regionSched chan<- worker.Task, tag string) (*PeerStorage, error) {
	...
	fmt.Printf("new peerStorage [%v] is created\n", tag)
	return &PeerStorage{
		Engines:     engines,
		region:      region,
		Tag:         tag,
...
func (ps *PeerStorage) ApplySnapshot(snapshot *eraftpb.Snapshot, kvWB *engine_util.WriteBatch, raftWB *engine_util.WriteBatch) (*ApplySnapResult, error) {
	...
	// Your Code Here (2C).
	// update the raftState, applyState
	...
	// update PeerStorage.snapState to snap.SnapState_Applying
	ps.snapState.StateType = snap.SnapState_Applying
	// update the regionState
	meta.WriteRegionState(kvWB, snapData.Region, rspb.PeerState_Normal)
	err1 := raftWB.SetMeta(meta.RaftStateKey(snapData.Region.Id), ps.raftState)
	if err1 != nil {
		panic(err1)
	}
	err2 := kvWB.SetMeta(meta.ApplyStateKey(snapData.Region.Id), ps.applyState)
	if err2 != nil {
		panic(err2)
	}
	// send runner.RegionTaskApply task to region worker through PeerStorage.regionSched and wait
...
func (ps *PeerStorage) SaveReadyState(ready *raft.Ready) (*ApplySnapResult, error) {
	...
	if !raft.IsEmptySnap(&ready.Snapshot) {
		raftWBForSnap := new(engine_util.WriteBatch)
		kvWBForSnap := new(engine_util.WriteBatch)
		result, _ = ps.ApplySnapshot(&ready.Snapshot, kvWBForSnap, raftWBForSnap)
		err = raftWBForSnap.WriteToDB(ps.Engines.Raft)
		err = kvWBForSnap.WriteToDB(ps.Engines.Kv)
	}
	raftWB := new(engine_util.WriteBatch)
	// 1. append log entries
	err = ps.Append(ready.Entries, raftWB)
	// 2. update the ps's raftLocalState and save it to raftDB
